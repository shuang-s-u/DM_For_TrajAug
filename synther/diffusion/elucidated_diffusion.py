...
class ElucidatedDiffusion(nn.Module):
    ...
    def preconditioned_network_forward(self, noised_inputs, sigma, clamp=False, cond=None):
        ...
        net_out = self.net(
            self.c_in(padded_sigma.to(config.device))* noised_inputs.to(config.device),
            self.c_noise(sigma.to(config.device)),
            cond=cond,
        )
    ...
    def compute_action_guidance(self, obs, action, normalize_action_guidance = True):
        
        # get pi from agent
        _ , mean, _ , _ , std, _  = self.policy.forward(obs)
        pi = Normal(mean,std)
        
        def transformed_action_log_prob(act):
            act_tanh = torch.tanh(act)  
            return pi.log_prob(act_tanh).sum() 
        
        # calculate action guidance by grad
        action = action.detach()
        action.requires_grad = True
        transformed_action_log_prob(action).backward()  
        action_guidance = action.grad.clone()
        if normalize_action_guidance:  
            action_guidance = action_guidance / action_guidance.norm() + 1e-8  
  
        return action_guidance

    def apply_guidance(self, inputs, step_idx, n_steps, obs_dim, action_dim):
        ...
        step_idx_tensor = torch.tensor(step_idx, dtype=torch.float32).to(config.device)  
        n_steps_tensor = torch.tensor(n_steps, dtype=torch.float32).to(config.device)  

        # lambd is the weight of guidance
        lambd = 1.0 - (step_idx / n_steps)  
        cosine_adjustment = torch.sin(torch.pi * ((step_idx_tensor + 1) / n_steps_tensor))  
        lambd += policy_guidance_cosine_coeff * cosine_adjustment  
        do_apply_guidance_this_step = (step_idx >= policy_guidance_delay_steps) & (step_idx < n_steps - 1)
        condition = torch.tensor(do_apply_guidance_this_step, dtype=torch.bool).expand_as(lambd)  
        condition = condition.to(config.device)  
        lambd = torch.where(condition, policy_guidance_coeff * lambd, torch.zeros_like(lambd))

        obs = inputs[:, :obs_dim]
        action = inputs[:, obs_dim:obs_dim+action_dim]        
        action_guidance = self.compute_action_guidance(obs, action)     
        guided_action = action + lambd * action_guidance
        guidance_inputs = inputs.clone()
        guidance_inputs[:, obs_dim:obs_dim+action_dim] = guided_action

        return guidance_inputs
    ...
    def sample(
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas, desc='sampling time step', mininterval=1,
            ...
            # add policy guidance
            guided_inputs = self.apply_guidance(inputs, denoising_step, num_sample_steps, self.obs_dim, self.action_dim)
    ...
    def log_likelihood(self, x, atol=1e-4, rtol=1e-4, clamp=False, normalize=True, cond=None):
        ...
        t = x.new_tensor([self.sigma_min, self.sigma_max]).to(config.device)  
    ...
    def forward(self, inputs, cond=None):
        ...
        padded_sigmas = padded_sigmas.to(config.device)
...
class Trainer(object):
    def __init__(
        ...
        self.model = diffusion_model.to(config.device)  
