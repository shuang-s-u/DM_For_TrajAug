...
class RandomOrLearnedSinusoidalPosEmb(nn.Module):
    ...
    def __init__(
        ...
        assert (dim % 2) == 0
        half_dim = dim // 2
        self.weights = nn.Parameter(torch.randn(half_dim, device=config.device), requires_grad=not is_random)
...
class ResidualBlock(nn.Module):
    def __init__(self, dim_in: int, dim_out: int, activation: str = "relu", layer_norm: bool = True):
        super().__init__()
        self.linear = nn.Linear(dim_in, dim_out, bias=True).to(config.device)
        if layer_norm:
...
class ResidualMLP(nn.Module):
    def __init__(
        ...
        self.network = nn.Sequential(
            nn.Linear(input_dim, width).to(config.device),
        ...
        self.activation = getattr(F, activation)
        self.final_linear = nn.Linear(width, output_dim).to(config.device)
...
class ResidualMLPDenoiser(nn.Module):
    def __init__(
        ...
        if cond_dim is not None:
            self.proj = nn.Linear(d_in + cond_dim, dim_t).to(config.device)
            self.conditional = True
        else:
            self.proj = nn.Linear(d_in, dim_t).to(config.device)
            self.conditional = False

        # time embeddings
        self.random_or_learned_sinusoidal_cond = learned_sinusoidal_cond or random_fourier_features
        if self.random_or_learned_sinusoidal_cond:
            sinu_pos_emb = RandomOrLearnedSinusoidalPosEmb(learned_sinusoidal_dim, random_fourier_features).to(config.device)
            sinu_pos_emb = sinu_pos_emb.to(config.device)
            fourier_dim = learned_sinusoidal_dim + 1
        else:
            sinu_pos_emb = SinusoidalPosEmb(dim_t).to(config.device)
            fourier_dim = dim_t

        self.time_mlp = nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, dim_t).to(config.device),
            nn.SiLU(),
            nn.Linear(dim_t, dim_t).to(config.device)
        )
    ...
    def forward(
        ...
        time_embed = self.time_mlp(timesteps).to(config.device)
